...
geom_point(color = "#0073C2FF", size = 1.8) +
scale_x_date(
date_breaks = "1 month",
date_labels = "%b %Y"
) +
...
coord_flip() +
labs(
title = "Top 20 Workshops by Number of Unique Participants",
x = "Workshop",
y = "Unique Participants"
) +
theme_minimal(base_size = 12) +
...
geom_col(fill = "#0073C2FF", width = 0.7) +
geom_text(aes(label = n_participants), vjust = -0.3, size = 2) +
theme_minimal(base_size = 12) +
labs(
...
org_summary %>%
  ...
  geom_text(aes(label = unique_participants),
            hjust = -0.1, size = 3) +
  coord_flip() +
  labs(
...
attendance_with_prices %>%
  ...
  labs(
    title = "Workshop Category Spectrum by Organisation",
    x = "Workshop Category",
    y = "Organisation"
  ) +
...
attendance_with_prices %>%
count(member_status) %>%
ggplot(aes(x = member_status, y = n, fill = member_status)) +
geom_col(width = 0.7) +
...
geom_col(width = 0.6) +
geom_text(aes(label = scales::comma(total_revenue)), vjust = -0.5, size = 3) +
labs(
title = "Total Revenue Comparison: 2024 vs 2025",
x = "Year",
y = "Total Revenue (AUD)"
) +
theme_minimal(base_size = 12) +
...
attendance_with_prices %>%
  ...
  coord_flip() +
  labs(
    title = "Number of Participants by Workshop Category",
    x = "Workshop Category",
    y = "Number of Participants"
  ) +
  theme_minimal()
...
```{r}
# ---- Create Table: Unique participants per month per year ----
